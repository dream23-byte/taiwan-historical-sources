# ============================================================
# Import-KoreaToSupabase.ps1 v1（純增量模式）
# 將韓國史料 CSV 匯入 Supabase（產生 INSERT SQL）
# Metadata v1.3 → 正規化資料庫（region=KR）
#
# 用法：
#   .\Import-KoreaToSupabase.ps1                                # 處理所有韓國 CSV
#   .\Import-KoreaToSupabase.ps1 -CsvFile "metadata_韓國.csv"   # 只處理單一檔案
#   .\Import-KoreaToSupabase.ps1 -DryRun                        # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_korea_sources.sql"
$CsvDir      = Join-Path $OutputDir "韓國"
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有韓國 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '韓國史前*'    { $periodId = 46; break }
            '高麗*'        { $periodId = 47; break }
            '朝鮮*'        { $periodId = 48; break }
            '大韓帝國*'    { $periodId = 49; break }
            '大韓民國前期*' { $periodId = 50; break }
            '大韓民國後期*' { $periodId = 51; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 韓國史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-KoreaToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（韓國時期） ---
$sb.AppendLine("-- === 0. 韓國時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (46, 'kr-ancient', '韓國史前・古代（～918）', NULL, 918, 49, 'KR'),")
$sb.AppendLine("  (47, 'kr-goryeo', '高麗（918–1392）', 918, 1392, 50, 'KR'),")
$sb.AppendLine("  (48, 'kr-joseon', '朝鮮（1392–1897）', 1392, 1897, 51, 'KR'),")
$sb.AppendLine("  (49, 'kr-empire-colonial', '大韓帝國・日治（1897–1945）', 1897, 1945, 52, 'KR'),")
$sb.AppendLine("  (50, 'kr-republic-pre', '大韓民國前期（1945–1987）', 1945, 1987, 53, 'KR'),")
$sb.AppendLine("  (51, 'kr-republic-post', '大韓民國後期（1987–迄今）', 1987, NULL, 54, 'KR')")
