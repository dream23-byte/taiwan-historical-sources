# ============================================================
# Import-SEAExtraToSupabase.ps1 v1（純增量模式）
# 將東南亞補充史料（寮國/柬埔寨/馬來西亞/新加坡）匯入 Supabase
# Metadata v1.3 → 正規化資料庫（region=SEA）
#
# 用法：
#   .\Import-SEAExtraToSupabase.ps1                          # 處理補充 CSV
#   .\Import-SEAExtraToSupabase.ps1 -CsvFile "..."           # 只處理單一檔案
#   .\Import-SEAExtraToSupabase.ps1 -DryRun                  # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_sea_sources_v2.sql"
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有補充 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '柬埔寨*'        { $periodId = 64; break }
            '寮國*'          { $periodId = 65; break }
            '馬來西亞*'      { $periodId = 66; break }
            '新加坡*'        { $periodId = 67; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 東南亞補充史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-SEAExtraToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（補充國家時期） ---
$sb.AppendLine("-- === 0. 補充國家時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (64, 'kh-cambodia', '柬埔寨（吳哥王朝～近代）', 802, 1953, 67, 'SEA'),")
$sb.AppendLine("  (65, 'la-laos', '寮國（瀾滄王國～近代）', 1353, 1975, 68, 'SEA'),")
$sb.AppendLine("  (66, 'my-malaysia', '馬來西亞（馬六甲蘇丹國～獨立）', 1400, 1957, 69, 'SEA'),")
$sb.AppendLine("  (67, 'sg-singapore', '新加坡（開埠～獨立）', 1819, 1965, 70, 'SEA')")
