# ============================================================
# Import-IndiaToSupabase.ps1 v1（純增量模式）
# 將印度史料（南亞第一站）匯入 Supabase
# Metadata v1.3 → 正規化資料庫（region=SA）
#
# 用法：
#   .\Import-IndiaToSupabase.ps1                          # 處理印度 CSV
#   .\Import-IndiaToSupabase.ps1 -CsvFile "..."           # 只處理單一檔案
#   .\Import-IndiaToSupabase.ps1 -DryRun                  # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_india_sources.sql"
$CsvDir      = Join-Path $OutputDir "印度"
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有印度 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '印度河流域*'        { $periodId = 68; break }
            '列國*'              { $periodId = 69; break }
            '中世紀印度*'        { $periodId = 70; break }
            '莫臥兒*'            { $periodId = 71; break }
            '英屬印度*'          { $periodId = 72; break }
            '印度共和*'          { $periodId = 73; break }
...
# --- 0. INSERT periods（印度 6 期） ---
$sb.AppendLine("-- === 0. 印度時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (68, 'in-valley-vedic', '印度河流域與吠陀時代（約前3000–前500）', -3000, -500, 71, 'SA'),")
$sb.AppendLine("  (69, 'in-mauryagupta', '列國・孔雀・笈多時代（約前500–550）', -500, 550, 72, 'SA'),")
$sb.AppendLine("  (70, 'in-medieval', '中世紀印度（德里蘇丹國，約550–1526）', 550, 1526, 73, 'SA'),")
$sb.AppendLine("  (71, 'in-mughal', '莫臥兒帝國（1526–1857）', 1526, 1857, 74, 'SA'),")
$sb.AppendLine("  (72, 'in-british', '英屬印度（1858–1947）', 1858, 1947, 75, 'SA'),")
$sb.AppendLine("  (73, 'in-republic', '印度共和（1947–迄今）', 1947, 2026, 76, 'SA')")
