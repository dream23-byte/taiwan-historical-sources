# ============================================================
# Import-JapanToSupabase.ps1 v1（純增量模式）
# 將日本史料 CSV 匯入 Supabase（產生 INSERT SQL）
# Metadata v1.3 → 正規化資料庫（region=JP）
#
# 用法：
#   .\Import-JapanToSupabase.ps1                                # 處理所有日本 CSV
#   .\Import-JapanToSupabase.ps1 -CsvFile "metadata_日本.csv"   # 只處理單一檔案
#   .\Import-JapanToSupabase.ps1 -DryRun                        # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_japan_sources.sql"
$CsvDir      = Join-Path $OutputDir "日本"
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有日本 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '日本史前*'    { $periodId = 39; break }
            '中世*'        { $periodId = 40; break }
            '江戶*'        { $periodId = 41; break }
            '明治*'        { $periodId = 42; break }
            '大正*'        { $periodId = 43; break }
            '昭和前期*'    { $periodId = 44; break }
            '戰後日本*'    { $periodId = 45; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 日本史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-JapanToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（日本時期） ---
$sb.AppendLine("-- === 0. 日本時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (39, 'jpn-ancient', '日本史前・古代（～1192）', NULL, 1192, 42, 'JP'),")
$sb.AppendLine("  (40, 'jpn-medieval', '中世（1192–1603）', 1192, 1603, 43, 'JP'),")
$sb.AppendLine("  (41, 'jpn-edo', '江戶（1603–1868）', 1603, 1868, 44, 'JP'),")
$sb.AppendLine("  (42, 'jpn-meiji', '明治（1868–1912）', 1868, 1912, 45, 'JP'),")
$sb.AppendLine("  (43, 'jpn-taisho', '大正（1912–1926）', 1912, 1926, 46, 'JP'),")
$sb.AppendLine("  (44, 'jpn-showa-pre', '昭和前期（1926–1945）', 1926, 1945, 47, 'JP'),")
$sb.AppendLine("  (45, 'jpn-postwar', '戰後日本（1945–迄今）', 1945, NULL, 48, 'JP')")
