# ============================================================
# Import-HKMoToSupabase.ps1 v1（純增量模式）
# 將港澳史料 CSV 匯入 Supabase（產生 INSERT SQL）
# Metadata v1.3 → 正規化資料庫（region=HK / region=MO）
#
# 用法：
#   .\Import-HKMoToSupabase.ps1                                # 處理所有港澳 CSV
#   .\Import-HKMoToSupabase.ps1 -CsvFile "metadata_香港.csv"   # 只處理單一檔案
#   .\Import-HKMoToSupabase.ps1 -DryRun                        # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_hkmo_sources.sql"
$CsvDir      = Join-Path $OutputDir "港澳"
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有港澳 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '香港英治*' { $periodId = 27; break }
            '香港日佔*' { $periodId = 28; break }
            '香港戰後*' { $periodId = 29; break }
            '香港特區*' { $periodId = 30; break }
            '澳門葡治*' { $periodId = 31; break }
            '澳門特區*' { $periodId = 32; break }
        ...
        # === region 依識別碼前綴判斷 ===
        $region = 'OTHER'
        if ($row.識別碼 -match '^HK-') { $region = 'HK' }
        elseif ($row.識別碼 -match '^MO-') { $region = 'MO' }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 港澳史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-HKMoToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（港澳時期） ---
$sb.AppendLine("-- === 0. 港澳時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (27, 'hk-british', '香港英治（1841–1941）', 1841, 1941, 30, 'HK'),")
$sb.AppendLine("  (28, 'hk-ww2', '香港日佔（1941–1945）', 1941, 1945, 31, 'HK'),")
$sb.AppendLine("  (29, 'hk-postwar', '香港戰後（1945–1997）', 1945, 1997, 32, 'HK'),")
$sb.AppendLine("  (30, 'hk-sar', '香港特區（1997–迄今）', 1997, NULL, 33, 'HK'),")
$sb.AppendLine("  (31, 'mo-portuguese', '澳門葡治（1557–1999）', 1557, 1999, 34, 'MO'),")
$sb.AppendLine("  (32, 'mo-sar', '澳門特區（1999–迄今）', 1999, NULL, 35, 'MO')")
