# ============================================================
# Import-RyukyuToSupabase.ps1 v1（純增量模式）
# 將琉球史料 CSV 匯入 Supabase（產生 INSERT SQL）
# Metadata v1.3 → 正規化資料庫（region=RY）
#
# 用法：
#   .\Import-RyukyuToSupabase.ps1                                # 處理所有琉球 CSV
#   .\Import-RyukyuToSupabase.ps1 -CsvFile "metadata_琉球.csv"   # 只處理單一檔案
#   .\Import-RyukyuToSupabase.ps1 -DryRun                        # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_ryukyu_sources.sql"
$CsvDir      = Join-Path $OutputDir "琉球"
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有琉球 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '古琉球*'        { $periodId = 33; break }
            '第一尚氏*'      { $periodId = 34; break }
            '第二尚氏*'      { $periodId = 35; break }
            '沖繩縣*'        { $periodId = 36; break }
            '琉球政府*'      { $periodId = 37; break }
            '沖繩返還後*'    { $periodId = 38; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 琉球史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-RyukyuToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（琉球時期） ---
$sb.AppendLine("-- === 0. 琉球時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (33, 'ryu-sanzan', '古琉球・三山時代（～1429）', NULL, 1429, 36, 'RY'),")
$sb.AppendLine("  (34, 'ryu-shoshou', '第一尚氏王朝（1429–1469）', 1429, 1469, 37, 'RY'),")
$sb.AppendLine("  (35, 'ryu-nishou', '第二尚氏王朝（1469–1879）', 1469, 1879, 38, 'RY'),")
$sb.AppendLine("  (36, 'oki-prefecture', '沖繩縣・日本統治（1879–1945）', 1879, 1945, 39, 'RY'),")
$sb.AppendLine("  (37, 'oki-us', '琉球政府・美治時期（1945–1972）', 1945, 1972, 40, 'RY'),")
$sb.AppendLine("  (38, 'oki-post-reversion', '沖繩返還後（1972–迄今）', 1972, NULL, 41, 'RY')")
