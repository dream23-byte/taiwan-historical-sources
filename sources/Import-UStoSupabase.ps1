# ============================================================
# Import-UStoSupabase.ps1 v1.1（純增量模式）
# 將美國史料 CSV 匯入 Supabase（產生 INSERT SQL）
# Metadata v1.4 → 正規化資料庫（region=US）｜新增 search_hint（檢索指引）欄位
#
# 用法：
#   .\Import-UStoSupabase.ps1                                # 處理所有美國 CSV
#   .\Import-UStoSupabase.ps1 -CsvFile "metadata_美國.csv"  # 只處理單一檔案
#   .\Import-UStoSupabase.ps1 -DryRun                        # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_us_sources.sql"
$CsvDir      = Join-Path $OutputDir "美國"
...
function Is-ValidInstitution {
    ...
    $institutionKeywords = @(
        '圖書館', '博物館', '博物院', '檔案館', '檔案處', '研究所', '大學', '學院'
        '文獻館', '書局', '出版社', '出版公司', '中心'
        '基金會', '委員會', '文化局', '文化處', '文化資產'
        '公報社', '氣象局', '天文台', '資料館', '辦事處', '教區'
        '數位', '檔案', '史料', '文獻'
        '國家', '國立', '市立', '縣立'
        '政府', '總督府', '殖民', '行'
        '國會', '海關', '海軍', '軍醫院', '水文'
        '社科院', '研究院', '研究與保護中心'
        'GIS', 'THDL', 'Wellcome'
        '縣廳', '教育委員會', '編集委員會', '調查會', '空軍'
        '公文書', '文化廳'
        '文化財', '記錄院', '編纂委員會', '海印寺', '寺'
        'Administration', 'Library', 'Museum', 'Institution', 'Foundation'
        'Association', 'Office', 'Bureau', 'Department', 'Agency'
        'University', 'College', 'Center', 'Institute'
    )
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有美國 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '1776*'        { $periodId = 113; break }
            '1775*'        { $periodId = 113; break }
            '1763*'        { $periodId = 113; break }
            '1800*'        { $periodId = 114; break }
            '1820*'        { $periodId = 114; break }
            '1861*'        { $periodId = 115; break }
            '1929*'        { $periodId = 117; break }
            '1939*'        { $periodId = 117; break }
            '1947*'        { $periodId = 118; break }
            '1955*'        { $periodId = 118; break }
            '1954*'        { $periodId = 118; break }
            '1950*'        { $periodId = 118; break }
            '1940*'        { $periodId = 118; break }
            '1921*'        { $periodId = 118; break }
            '1970*'        { $periodId = 119; break }
            '1972*'        { $periodId = 119; break }
            '1973*'        { $periodId = 119; break }
            '1996*'        { $periodId = 119; break }
            '2000*'        { $periodId = 119; break }
            '1846*'        { $periodId = 116; break }
            '1872*'        { $periodId = 116; break }
            '1873*'        { $periodId = 116; break }
            '1777*'        { $periodId = 113; break }
            '1790*'        { $periodId = 114; break }
            '1600*'        { $periodId = 113; break }
            '1700*'        { $periodId = 114; break }
            '古代*'        { $periodId = 113; break }
            '1940s*'       { $periodId = 118; break }
            '1950s*'       { $periodId = 118; break }
        ...
        $allSources += [PSCustomObject]@{
            ...
            search_hint = if ($row.檢索指引 -and $row.檢索指引 -ne '–' -and $row.檢索指引 -ne '﹣' -and $row.檢索指引 -ne '') { $row.檢索指引 } else { $null }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 美國史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-UStoSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（美國時期） ---
$sb.AppendLine("-- === 0. 美國時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (113, 'us-colonial-revolution', '殖民地與獨立時期（1607–1789）', 1607, 1789, 113, 'US'),")
$sb.AppendLine("  (114, 'us-early-republic', '早期共和國（1789–1861）', 1789, 1861, 114, 'US'),")
$sb.AppendLine("  (115, 'us-civil-war', '南北戰爭時期（1861–1865）', 1861, 1865, 115, 'US'),")
$sb.AppendLine("  (116, 'us-reconstruction-gilded', '重建與鍍金時代（1865–1929）', 1865, 1929, 116, 'US'),")
$sb.AppendLine("  (117, 'us-great-depression-ww2', '大蕭條與二戰（1929–1945）', 1929, 1945, 117, 'US'),")
$sb.AppendLine("  (118, 'us-cold-war', '冷戰時期（1945–1991）', 1945, 1991, 118, 'US'),")
$sb.AppendLine("  (119, 'us-modern', '當代美國（1991–迄今）', 1991, NULL, 119, 'US')")
...
foreach ($s in $allSources) {
    ...
    $cols = @('title_zh','creator','date','source_name','identifier','source_url',
              ...
              'license_status','license_note','collection_status','source_description','notes','search_hint')
    ...
    $sql = "INSERT INTO sources (id, title_zh, creator, date, period_id, source_type_id, source_name, identifier, source_url, language, description, subject, era_range, geo_range, translator, relation, publisher, contributor, access_date, license_status, license_note, collection_status, source_description, notes, search_hint, region) OVERRIDING SYSTEM VALUE VALUES ($sourceId, $($vals[0]), $($vals[1]), $($vals[2]), $periodVal, $typeVal, $($vals[3]), $($vals[4]), $($vals[5]), $($vals[6]), $($vals[7]), $($vals[8]), $($vals[9]), $($vals[10]), $($vals[11]), $($vals[12]), $($vals[13]), $($vals[14]), $($vals[15]), $($vals[16]), $($vals[17]), $($vals[18]), $($vals[19]), $($vals[20]), $($vals[21]), '$($s.region)') ON CONFLICT (id) DO NOTHING;"
    $sb.AppendLine($sql)
...
if ($uniqueRows.Count -gt 0) {
    for ($i = 0; $i -lt $uniqueRows.Count; $i += 50) {
        ...
        for ($j = 0; $j -lt $batch.Count; $j++) {
            $comma = if ($j -lt $batch.Count - 1) { ',' } else { '' }
