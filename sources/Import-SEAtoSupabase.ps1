# ============================================================
# Import-SEAtoSupabase.ps1 v1（純增量模式）
# 將東南亞史料 CSV 匯入 Supabase（產生 INSERT SQL）
# Metadata v1.3 → 正規化資料庫（region=SEA）
#
# 用法：
#   .\Import-SEAtoSupabase.ps1                                # 處理所有東南亞 CSV
#   .\Import-SEAtoSupabase.ps1 -CsvFile "metadata_東南亞.csv"  # 只處理單一檔案
#   .\Import-SEAtoSupabase.ps1 -DryRun                        # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_sea_sources.sql"
$CsvDir      = Join-Path $OutputDir "東南亞"
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理所有東南亞 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '越南古代*'        { $periodId = 52; break }
            '李陳黎朝*'        { $periodId = 53; break }
            '阮朝*'            { $periodId = 54; break }
            '法屬印度支那*'    { $periodId = 55; break }
            '越南現代*'        { $periodId = 56; break }
            '前殖民時期*'      { $periodId = 57; break }
            '西屬時期*'        { $periodId = 58; break }
            '美屬時期*'        { $periodId = 59; break }
            '菲律賓共和*'      { $periodId = 60; break }
            '泰國*'            { $periodId = 61; break }
            '緬甸*'            { $periodId = 62; break }
            '印尼*'            { $periodId = 63; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 東南亞史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-SEAtoSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（東南亞時期） ---
$sb.AppendLine("-- === 0. 東南亞時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (52, 'vn-ancient', '越南古代・北屬（～939）', NULL, 939, 55, 'SEA'),")
$sb.AppendLine("  (53, 'vn-lytranle', '李陳黎朝（939–1527）', 939, 1527, 56, 'SEA'),")
$sb.AppendLine("  (54, 'vn-nguyen', '阮朝（1802–1945）', 1802, 1945, 57, 'SEA'),")
$sb.AppendLine("  (55, 'vn-french-indochina', '法屬印度支那（1858–1945）', 1858, 1945, 58, 'SEA'),")
$sb.AppendLine("  (56, 'vn-modern', '越南現代（1945–迄今）', 1945, NULL, 59, 'SEA'),")
$sb.AppendLine("  (57, 'ph-precolonial', '前殖民時期（～1565）', NULL, 1565, 60, 'SEA'),")
$sb.AppendLine("  (58, 'ph-spanish', '西屬時期（1565–1898）', 1565, 1898, 61, 'SEA'),")
$sb.AppendLine("  (59, 'ph-american', '美屬時期（1898–1946）', 1898, 1946, 62, 'SEA'),")
$sb.AppendLine("  (60, 'ph-republic', '菲律賓共和（1946–迄今）', 1946, NULL, 63, 'SEA'),")
$sb.AppendLine("  (61, 'th-thailand', '泰國（素可泰～曼谷王朝）', 1238, NULL, 64, 'SEA'),")
$sb.AppendLine("  (62, 'mm-myanmar', '緬甸（蒲甘～貢榜王朝）', 849, 1885, 65, 'SEA'),")
$sb.AppendLine("  (63, 'id-indonesia', '印尼（滿者伯夷～獨立）', 1293, NULL, 66, 'SEA')")
