# ============================================================
# Import-BhutanToSupabase.ps1 v1（純增量模式）
# 將不丹史料（南亞第五站）匯入 Supabase
# Metadata v1.3 → 正規化資料庫（region=SA）
#
# 用法：
#   .\Import-BhutanToSupabase.ps1                       # 處理不丹 CSV
#   .\Import-BhutanToSupabase.ps1 -CsvFile "..."        # 只處理單一檔案
#   .\Import-BhutanToSupabase.ps1 -DryRun               # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_bhutan_sources.sql"
$CsvDir      = $OutputDir
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理不丹 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '古代王朝*'      { $periodId = 124; break }
            '沙布瓊*'        { $periodId = 125; break }
            '不丹王國*'      { $periodId = 126; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 不丹史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-BhutanToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（不丹 3 期） ---
$sb.AppendLine("-- === 0. 不丹時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (124, 'bt-early-buddhist', '不丹：佛教傳入與前統一時期（約7世紀–1616）', 600, 1616, 97, 'SA'),")
$sb.AppendLine("  (125, 'bt-shabdrung', '不丹：沙布瓊統一與座宗時代（1616–1907）', 1616, 1907, 98, 'SA'),")
$sb.AppendLine("  (126, 'bt-wangchuck', '不丹：旺楚克王朝與憲政民主（1907–迄今）', 1907, 2026, 99, 'SA')")
$sb.AppendLine("ON CONFLICT (id) DO NOTHING;")
$sb.AppendLine()
$sb.AppendLine("SELECT setval('periods_id_seq', 126);")
$sb.AppendLine()
