# ============================================================
# Import-SouthAsiaToSupabase.ps1 v1（純增量模式）
# 將巴基斯坦・孟加拉・斯里蘭卡史料（南亞第二站）匯入 Supabase
# Metadata v1.3 → 正規化資料庫（region=SA）
#
# 用法：
#   .\Import-SouthAsiaToSupabase.ps1                       # 處理三國 CSV
#   .\Import-SouthAsiaToSupabase.ps1 -CsvFile "..."        # 只處理單一檔案
#   .\Import-SouthAsiaToSupabase.ps1 -DryRun               # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_south_asia_sources.sql"
$CsvDir      = $OutputDir
...
# === 決定要處理哪些 CSV ===
$allCsvFiles = @(
    "巴基斯坦\metadata_巴基斯坦.csv"
    "孟加拉\metadata_孟加拉.csv"
    "斯里蘭卡\metadata_斯里蘭卡.csv"
)

if ($CsvFile) {
    if (-not (Test-Path (Join-Path $CsvDir $CsvFile))) {
        Write-Error "找不到檔案: $CsvFile"
        exit 1
    }
    $CsvFiles = @($CsvFile)
    ...
    Write-Host "完整模式：處理所有南亞 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '印度河流域*'        { $periodId = 74; break }
            '犍陀羅*'            { $periodId = 75; break }
            '莫臥兒*'            { $periodId = 76; break }
            '英屬印度*'          { $periodId = 77; break }
            '巴基斯坦國*'        { $periodId = 78; break }
            '古孟加拉*'          { $periodId = 79; break }
            '伊斯蘭蘇丹*'        { $periodId = 80; break }
            '英屬孟加拉*'        { $periodId = 81; break }
            '東巴基斯坦*'        { $periodId = 82; break }
            '孟加拉國*'          { $periodId = 83; break }
            '古斯里蘭卡*'        { $periodId = 84; break }
            '康提*'              { $periodId = 85; break }
            '荷蘭殖民*'          { $periodId = 86; break }
            '葡荷殖民*'          { $periodId = 86; break }
            '英屬錫蘭*'          { $periodId = 87; break }
            '斯里蘭卡*'          { $periodId = 88; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 印度史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-IndiaToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（南亞三國 15 期） ---
$sb.AppendLine("-- === 0. 南亞三國時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (74, 'pk-indus-valley', '巴基斯坦：印度河流域文明（約前3300–前1300）', -3300, -1300, 77, 'SA'),")
$sb.AppendLine("  (75, 'pk-gandhara', '巴基斯坦：犍陀羅・佛教時代（約前500–550）', -500, 550, 78, 'SA'),")
$sb.AppendLine("  (76, 'pk-islamic', '巴基斯坦：伊斯蘭王朝與蒙兀兒（約712–1857）', 712, 1857, 79, 'SA'),")
$sb.AppendLine("  (77, 'pk-british', '巴基斯坦：英屬印度（1858–1947）', 1858, 1947, 80, 'SA'),")
$sb.AppendLine("  (78, 'pk-republic', '巴基斯坦國（1947–迄今）', 1947, 2026, 81, 'SA'),")
$sb.AppendLine("  (79, 'bd-ancient', '孟加拉：古孟加拉・帕拉王朝（約前3世紀–1204）', -300, 1204, 82, 'SA'),")
$sb.AppendLine("  (80, 'bd-islamic', '孟加拉：伊斯蘭蘇丹與莫臥兒（1204–1757）', 1204, 1757, 83, 'SA'),")
$sb.AppendLine("  (81, 'bd-british', '孟加拉：英屬孟加拉（1757–1947）', 1757, 1947, 84, 'SA'),")
$sb.AppendLine("  (82, 'bd-pakistan', '東巴基斯坦（1947–1971）', 1947, 1971, 85, 'SA'),")
$sb.AppendLine("  (83, 'bd-republic', '孟加拉國（1971–迄今）', 1971, 2026, 86, 'SA'),")
$sb.AppendLine("  (84, 'lk-ancient', '斯里蘭卡：古斯里蘭卡（阿努拉德普勒・波隆納魯瓦，約前6世紀–1232）', -600, 1232, 87, 'SA'),")
$sb.AppendLine("  (85, 'lk-kandy', '斯里蘭卡：康提王國（1592–1815）', 1592, 1815, 88, 'SA'),")
$sb.AppendLine("  (86, 'lk-portuguese', '斯里蘭卡：葡荷殖民（1505–1796）', 1505, 1796, 89, 'SA'),")
$sb.AppendLine("  (87, 'lk-british', '斯里蘭卡：英屬錫蘭（1796–1948）', 1796, 1948, 90, 'SA'),")
$sb.AppendLine("  (88, 'lk-republic', '斯里蘭卡共和（1948–迄今）', 1948, 2026, 91, 'SA')")
