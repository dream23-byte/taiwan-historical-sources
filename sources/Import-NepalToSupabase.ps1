# ============================================================
# Import-NepalToSupabase.ps1 v1（純增量模式）
# 將尼泊爾史料（南亞第四站）匯入 Supabase
# Metadata v1.3 → 正規化資料庫（region=SA）
#
# 用法：
#   .\Import-NepalToSupabase.ps1                       # 處理尼泊爾 CSV
#   .\Import-NepalToSupabase.ps1 -CsvFile "..."        # 只處理單一檔案
#   .\Import-NepalToSupabase.ps1 -DryRun               # 只看不執行
# ============================================================
...
$OutputSql   = Join-Path $OutputDir "import_nepal_sources.sql"
$CsvDir      = $OutputDir
...
if ($CsvFile) {
    ...
    Write-Host "完整模式：處理尼泊爾 CSV（已存在的會用 ON CONFLICT 跳過）"
}
...
foreach ($filename in $CsvFiles) {
    ...
    foreach ($row in $data) {
        ...
        $periodId = $null
        switch -Wildcard ($era) {
            '古代*'          { $periodId = 89; break }
            '馬拉王朝*'      { $periodId = 120; break }
            '沙阿王朝*'      { $periodId = 121; break }
            '拉納時期*'      { $periodId = 122; break }
            '近代～當代*'    { $periodId = 122; break }
            '尼泊爾共和*'    { $periodId = 123; break }
...
$sb.AppendLine("-- ============================================================")
$sb.AppendLine("-- 東亞歷史權威史料庫 — 尼泊爾史料匯入 SQL（增量模式）")
$sb.AppendLine("-- 由 Import-NepalToSupabase.ps1 v1 自動產生")
$sb.AppendLine("-- ============================================================")
$sb.AppendLine()

# --- 0. INSERT periods（尼泊爾 5 期） ---
$sb.AppendLine("-- === 0. 尼泊爾時期（僅首次執行） ===")
$sb.AppendLine("INSERT INTO periods (id, code, name_zh, start_year, end_year, sort_order, region) OVERRIDING SYSTEM VALUE VALUES")
$sb.AppendLine("  (89, 'np-ancient', '尼泊爾：古代王國（基拉特・李察維時期，約前7世紀–750）', -700, 750, 92, 'SA'),")
$sb.AppendLine("  (120, 'np-malla', '尼泊爾：馬拉王朝（750–1768）', 750, 1768, 93, 'SA'),")
$sb.AppendLine("  (121, 'np-shah', '尼泊爾：沙阿王朝統一（1768–1846）', 1768, 1846, 94, 'SA'),")
$sb.AppendLine("  (122, 'np-rana', '尼泊爾：拉納王朝（1846–1951）', 1846, 1951, 95, 'SA'),")
$sb.AppendLine("  (123, 'np-republic', '尼泊爾：君主立憲～聯邦共和（1951–迄今）', 1951, 2026, 96, 'SA')")
$sb.AppendLine("ON CONFLICT (id) DO NOTHING;")
$sb.AppendLine()
$sb.AppendLine("SELECT setval('periods_id_seq', 123);")
$sb.AppendLine()
